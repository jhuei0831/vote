...
type PaginationService[T PaginationQuery, S any] struct {
}

func NewPaginationService[T PaginationQuery, S any]() PaginationService[T, S] {
	return PaginationService[T, S]{}
}

// Handler 處理分頁邏輯，根據提供的分頁參數修改 GORM 查詢。
func (p *PaginationService[T, S]) Handler(db *gorm.DB, query T) (*gorm.DB, error) {
	// 檢查分頁參數
...
func (p *PaginationService[T, S]) HasPreviousNextPage(items []S, query T) ([]S, bool, bool) {
...
func (p *PaginationService[T, S]) CheckPaginationParams(query T) error {
